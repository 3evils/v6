...
$HTMLOUT.= "<div class='row'><div class='medium-12 columns'>";
...
$i = 0;
    foreach ($cats as $cat) {
        if($cat['min_class'] <= $CURUSER['class']){
            $HTMLOUT.= ($i && $i % $INSTALLER09['catsperrow'] == 0) ? "</tr>" : "";
            $HTMLOUT.= "<td>
             <input name='c" . (int)$cat['id'] . "' class=\"input-group-field\" type=\"checkbox\" " . (in_array($cat['id'], $wherecatina) ? "checked='checked' " : "") . "value='1'><a class='catlink' href='browse.php?cat=" . (int)$cat['id'] . "'> " . (($CURUSER['opt2'] & user_options_2::BROWSE_ICONS) ? "<img src='{$INSTALLER09['pic_base_url']}caticons/{$CURUSER['categorie_icon']}/" . htmlsafechars($cat['image']) . "' alt='" . htmlsafechars($cat['name']) . "' title='" . htmlsafechars($cat['name']) . "'>" : "" . htmlsafechars($cat['name']) . "") . "</a></td>\n";
            $i++;
        }
    }
$alllink = "";
...
<div class='input-group height-50'>
	...
	<input class='input-group-field' placeholder='Search' type='text' name='search' value=''>";
//=== only free option :o)
$only_free =((isset($_GET['only_free'])) ? intval($_GET['only_free']) : '');
//=== checkbox for only free torrents
$only_free_box = '<input class="input-group-field" type="checkbox" name="only_free" value="1"'.(isset($_GET['only_free']) ? ' checked="checked"' : '').'><span class="input-group-label"><b>Only Free Torrents</b></span>';
$selected = (isset($_GET["incldead"])) ? (int)$_GET["incldead"] : "";
$deadcheck = "";
$deadcheck.= "<span class='input-group-label'><b>in:</b></span>
<select class='input-group-field' name='incldead'>
...
$searchin = '<span class="input-group-label height-25"><b>by:</b></span>
	<select class="input-group-field" name="searchin">';
foreach (array(
    'title' => 'Name',
    'descr' => 'Description',
    'genre' => 'Genre',
...
$HTMLOUT.= "<div class='input-group-button'><input class='input-group-button button' type='submit' value='{$lang['search_search_btn']}'></div></div>
</form>
...
if ($count) {
    $HTMLOUT.="<!--<br /><div class='row'><div class='medium-3 medium-offset-5 columns'><div style='display:inline-block;width:4px';></div><a href='{$INSTALLER09["baseurl"]}/catalogue.php' class='btn btn-default btn-default'>Search our Catalogue</a></div></div><br /><br />-->";
    $HTMLOUT.= "";
    $HTMLOUT.= torrenttable($res);
    $HTMLOUT.= $pager['pagerbottom']."<br />";
} else {
    if (isset($cleansearchstr)) {
        $HTMLOUT.= "<div class='callout small warning'><h2>{$lang['browse_not_found']}</h2>";
        $HTMLOUT.= "{$lang['browse_tryagain']}</div>";
    } else {
        $HTMLOUT.= "<div class='callout small warning'><h2>{$lang['browse_nothing']}</h2>\n";
        $HTMLOUT.= "{$lang['browse_sorry']}</div>";
    }
}
$HTMLOUT.= "";
$ip = getip();
