...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (sql_query('INSERT INTO tickets(user) VALUES ' . join(', ', $t))) {
        ...
        $What_Cache = (OCELOT_TRACKER == true ? 'userstats_xbt_' : 'userstats_');
        $What_Expire = (OCELOT_TRACKER == true ? $INSTALLER09['expires']['u_stats_xbt'] : $INSTALLER09['expires']['u_stats']);
        $mc1->begin_transaction($What_Cache . $CURUSER['id']);
        $mc1->update_row(false, array(
        'seedbonus' => $seedbonus_new
        ));
        $mc1->commit_transaction($What_Expire);
        $What_Cache = (OCELOT_TRACKER == true ? 'user_stats_xbt_' : 'user_stats_');
        $What_Expire = (OCELOT_TRACKER == true ? $INSTALLER09['expires']['user_stats_xbt'] : $INSTALLER09['expires']['user_stats']);
        $mc1->begin_transaction($What_Cache . $CURUSER['id']);
        $mc1->update_row(false, array(
        'seedbonus' => $seedbonus_new
        ));
...
if (!(is_array($classes_allowed) ? in_array($CURUSER['class'], $classes_allowed) : $CURUSER['class'] == $classes_allowed)) stderr('Error', 'Your class is not allowed to play in this lottery');
//some default values
$lottery['total_pot'] = 0;
...
if ($CURUSER['seedbonus'] < ($lottery['current_user']['could_buy'] * $lottery_config['ticket_amount'])) for ($lottery['current_user']['can_buy']; $CURUSER['seedbonus'] < ($lottery_config['ticket_amount'] * $lottery['current_user']['can_buy']); --$lottery['current_user']['can_buy']);
...
if ($lottery['current_user']['can_buy'] > 0) $html.= "
      <tr>
