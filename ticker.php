...
loggedinorreturn();
global $lang;
$HTMLOUT = $query = $dellall = $dtcolor = $fontcolor = $bg = $pm = $reply = $Beeper = $ignore = $del = $private = $edit = '';
...
<body bgcolor='#F5F4EA' class='date'>
<form method='post' action='./bot_shoutbox.php'>
<input type='hidden' name='id' value='" . (int)$res['id'] . "' />
...
<meta http-equiv='refresh' content='15; url=./bot_shoutbox.php' />
<script type='text/javascript' src='./scripts/shout2.js'></script>
<style type='text/css'>
...
if ($CURUSER['stylesheet'] == 2) {
    $HTMLOUT.= "<style type='text/css'>
A {color: #0085C2; font-weight: bold;  }
...
// == Black
if ($CURUSER['stylesheet'] == 6) {
    $HTMLOUT.= "<style type='text/css'>
A {color: #24c20b; font-weight: bold; ; }
...
if (($shouts = $mc1->get_value('auto_shoutbox_')) === false) {
    ...
    $mc1->cache_value('bot_shoutbox_', $shouts, $INSTALLER09['expires']['shoutbox']);
}
    ...
    if ($shouts) {
        ...
        foreach ($shouts as $arr) {
            ...
            $user_stuff['username'] = ($arr['perms'] & bt_options::PERMS_STEALTH ? "" . $user_stuff['username'] = 'UnKn0wn' . "" : "" . $user_stuff['username'] = htmlsafechars($arr['username']) . "");
						...
						$today = getdate("D g:i a"); // 03.10.01
						
			$HTMLOUT.= "<tr style='background-color:$bg;'><td>\n <span style='color:white;'>$date " . format_comment($arr["text"]) . "\n</span><span style='display: inline-block; text-align: right; width: 100%'></span></td></tr>\n";}
        $HTMLOUT.= "</table>";
    } else {
