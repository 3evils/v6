	...
	*/
	...
	if (!empty($_POST['to']) AND !empty($_POST['message'])) 
	{
		if (logged_in($userid)) 
		{
			...
			if (empty($disable_sending_private_msg))
			{
				$db->execute("
				...
				if ($push_on == 1)
				{
					push_publish('u' . $_POST['to'], array('messages' => array("id" => $last_id, "from" => $userid, "message" => sanitize($message), "sent" => time(), "self" => "0", "old" => "0")));
					
					push_publish('u' . $userid, array('messages' => array("id" => $last_id, "from" => $_POST['to'], "message" => sanitize($message), "sent" => time(), "self" => "1", "old" => "0")));
				}
