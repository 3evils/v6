<?php
	...
	*/
	...
	$chatroom_unfocus		= get_var('chatroom_unfocus');
	...
	if (!empty($_POST['unfocus_chat'])) 
	{
		...
		if ($result AND $db->count_select() > 0) 
		{
			...
			$unfocus_chat = $row['unfocus_chat'];
			$exploded_unfocus = explode(':', $unfocus_chat);
			
			if (!in_array($_POST['unfocus_chat'], $exploded_unfocus))
			{
	...
	// ##################### START POST CHATROOM UNFOCUS ########################
	if (var_check('chatroom_unfocus'))
	{
		$result = $db->execute("
			SELECT chatroom_unfocus, chatroom_stay
			FROM arrowchat_status 
			WHERE userid = '" . $db->escape_string($userid) . "'
		");
		...
		$db->execute("
			...
			WHERE user_id = '" . $db->escape_string($userid) . "'
				AND chatroom_id = '" . $db->escape_string($chatroom_unfocus) . "'
		");
		...
		if ($result AND $db->count_select() > 0) 
		{
			...
			$chatroom_stay_insert = $row['chatroom_stay'];
			$chatroom_unfocus_insert = $row['chatroom_unfocus'];
			
			if (@in_array($chatroom_unfocus, $chatroom_unfocus_array))
			{
				$chatroom_stay_insert = $row['chatroom_stay'];
				
				if (($key = array_search($chatroom_unfocus, $chatroom_unfocus_array)) !== false) 
				{
					unset($chatroom_unfocus_array[$key]);
				}
				
				$chatroom_unfocus_insert = serialize($chatroom_unfocus_array);
			}
			else if (trim($chatroom_unfocus) == trim($row['chatroom_stay']))
			{
				$chatroom_stay_insert = 0;
				$chatroom_unfocus_insert = $row['chatroom_unfocus'];
			}
		}
		
		// Temporary fix to prevent this query from running before others
		sleep(1);
		...
		$db->execute("
			...
			VALUES (
				'" . $db->escape_string($userid) . "',
				'" . $db->escape_string($chatroom_stay_insert) . "', 
				'" . $db->escape_string($chatroom_unfocus_insert) . "'
			) 
			ON DUPLICATE KEY 
				UPDATE chatroom_stay = '" . $db->escape_string(trim($chatroom_stay_insert)) . "', chatroom_unfocus = '" . $db->escape_string(trim($chatroom_unfocus_insert)) . "'
		");
	...
	if (var_check('chatroom_mod')) 
	{
		...
		if ($result AND $db->count_select() > 0) 
		{
			...
			if ($push_on == 1)
			{
				push_publish('chatroom' . $chatroom_id, array('chatroommessage' => array("id" => $db->last_insert_id(), "name" => 'Global', "message" => $mod_message, "userid" => $userid, "sent" => time(), "global" => '1', "chatroomid" => $chatroom_id)));
			}
	...
	if (var_check('chatroom_ban')) 
	{
		...
		if ($result AND $db->count_select() > 0) 
		{
			...
			if (empty($chatroom_ban_length))
				$ban_message2 = $language[55] . $chatroom_name;
			else
				$ban_message2 = $language[56] . $chatroom_name . $language[219] . $chatroom_ban_length . $language[220];
			
			if ($push_on == 1)
			{
				push_publish('chatroom' . $chatroom_id, array('chatroommessage' => array("id" => $db->last_insert_id(), "name" => 'Global', "message" => $ban_message, "userid" => $userid, "sent" => time(), "global" => '1', "chatroomid" => $chatroom_id)));
				
				push_publish('u' . $chatroom_ban, array('chatroomban' => array("error2" => $ban_message2, 'id' => $chatroom_id)));
			}
	...
	if (var_check('chatroom_silence')) 
	{
		...
		if ($result AND $db->count_select() > 0 AND is_numeric($chatroom_silence_length)) 
		{
			...
			if ($push_on == 1)
			{
				push_publish('chatroom' . $chatroom_id, array('chatroommessage' => array("id" => $db->last_insert_id(), "name" => 'Global', "message" => $silence_message, "userid" => $userid, "sent" => time(), "global" => '1', "chatroomid" => $chatroom_id)));
			}
	...
	if (var_check('delete_msg')) 
	{
		...
		if ($result AND $db->count_select() > 0) 
		{
			...
			if ($push_on == 1)
			{
				push_publish('chatroom' . $chatroom_id, array('chatroommessage' => array("id" => $delete_msg, "name" => 'Delete', "message" => $language[159] . $delete_name, "userid" => $userid, "sent" => time(), "global" => '1', "mod" => 0, "admin" => 0, "chatroomid" => $chatroom_id)));
			}
		}
		...
		{		
			...
			if ($row = $db->fetch_array($result))
			{
				...
				else
				{
					...
					if ($push_on == 1)
					{
						push_publish('u' . $row['report_about'], array('warning' => array("data" => $report_warn_reason, "read" => "0")));
					}
				}
			}
		}
