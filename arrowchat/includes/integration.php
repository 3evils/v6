	...
	function get_user_id() 
{
	...
	function get_link($link, $user_id) 
	{
		...
		return $base_url . '../userdetails.php?id=' . $link;
	}
	...
	//function get_avatar($image, $user_id)
       // {
        ...
        //$image = mysqli_query("SELECT avatar FROM users WHERE id = $userid");
        //$avatar_url = mysqli_use_result($image,0);
        //$avatar_url = $image;
       // return $image;
        //}
		...
		//function get_avatar($image, $user_id) 
	//{
		//$sql = mysqli_query("SELECT avatar FROM users WHERE id = $userid");
		//$image = $sql;
	//	return $image;
		//$image = mysqli_query("SELECT avatar FROM users WHERE id = $userid");
        //$image = mysqli_result($sql,0);
        //$avatar_url = $image;
       // return $image;
...
//	}
	function get_avatar($image, $user_id) 
	{
		...
		else if (is_file('../../images/avatars/gallery/' . $image)) 
		{
	...
	{	
		...
		{	 
			while ($row = $db->fetch_array($result))
			{
				...
				$groups[] = array($row['value'], $row['classname']);
			}
