...
function write_staffs()
{
    ...
    $ri = sql_query("SELECT id, username, class FROM users WHERE class BETWEEN " . UC_STAFF . " AND " . UC_CEO . " ORDER BY id ASC") or sqlerr(__file__, __line__);
...
{
    $class = (int)$class;
    return (bool)($class >= UC_MIN && $class <= UC_CEO);
}
function min_class($min = UC_MIN, $max = UC_CEO)
{
    global $CURUSER;
    $minclass = (int)$min;
    $maxclass = (int)$max;
...
{
    global $CURUSER, $INSTALLER09;
    $avatar_show = ($CURUSER['avatars'] == 'no' ? '' : (!$avatar['avatar'] ? '<img style="max-width:'.$width.'px;" src="'.$INSTALLER09['pic_base_url'].'default_avatar.png" alt="avatar" />' : (($avatar['offensive_avatar'] === 'yes' && $CURUSER['view_offensive_avatar'] === 'no') ? '<img style="max-width:'.$width.'px;" src="'.$INSTALLER09['pic_base_url'].'fuzzybunny.gif" alt="avatar" />' : '<img style="max-width:'.$width.'px;" src="'.htmlsafechars($avatar['avatar']).'" alt="avatar" />')));
    return $avatar_show;
}
