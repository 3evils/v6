<?php
	...
	*/
	function getChatroom() 
	{
		...
		foreach ($chatroomid as $key => $value)
		{
			$query = "
				SELECT m.id, m.username, m.message, m.sent, m.edited_session_id, m.global_message, m.user_id, m.is_mod, m.is_admin, m.action 
				FROM arrowchat_chatroom_messages AS m 
				JOIN arrowchat_chatroom_users AS u 
					ON u.user_id = '" . $db->escape_string($userid) . "'       
					AND u.chatroom_id = '" . $db->escape_string($value) . "' 
					AND u.session_time != 0 
				WHERE m.chatroom_id = '" . $db->escape_string($value) . "'      
					AND (" . $db->escape_string($time2) . " <= m.sent  OR m.edited >= " . $db->escape_string($time2) . ")     
					AND '" . $db->escape_string($userid) . "' != m.user_id  
				ORDER BY m.sent ASC
			";
			$querys = "SELECT m.id,m.edited_session_id,m.is_mod FROM arrowchat_chatroom_messages m where m.is_mod = '2' AND '" . $db->escape_string($userid) . "' != m.user_id";
			
			$result = $db->execute($query);
			while ($row = $db->fetch_array($result)) 
			{
				$edited = 0;
				if (!empty($row['edited_session_id']) && !in_array($row['edited_session_id'], $_SESSION['chatroom_mess_ids_edit'])){ 
					$key = array_search($row['id'], $_SESSION['chatroom_mess_ids']);
					unset($_SESSION['chatroom_mess_ids'][$key]);
					$_SESSION['chatroom_mess_ids_edit'][] = $row['edited_session_id'];
					//$edited = 1;
				}
				//$chatroom[] = array('id' => $row['id'], 'edit' => $edited, 'userid' => $row['user_id'], 'n' => $row['username'], 'm' => print_r($_SESSION['chatroom_mess_ids_edit']), 't' => $row['sent'], 'global' => $row['global_message'], 'mod' => $row['is_mod'], 'admin' => $row['is_admin'], "chatroomid" => $value, 'action' => $row['action']);
				if (!in_array($row['id'], $_SESSION['chatroom_mess_ids'])) 
				{
					...
					$chatroom[] = array('id' => $row['id'], 'edit' => $edited, 'userid' => $row['user_id'], 'n' => $row['username'], 'm' => $chat_message, 't' => $row['sent'], 'global' => $row['global_message'], 'mod' => $row['is_mod'], 'admin' => $row['is_admin'], "chatroomid" => $value, 'action' => $row['action']);
